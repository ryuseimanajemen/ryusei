/**
 * Ryusei Chatbot Engine v2.1 (Mistral AI Integration)
 * File ini berisi semua logika AI Chatbot Ryusei dengan integrasi Mistral AI.
...
(function () {
  ...
  const STATE = {
    ...
    mistralKey: null, // Kunci API Mistral
    endpoint: null, // Endpoint Mistral API
    cmsData: null,
    eventsData: null, // Cache event list untuk konteks lebih pintar
    configLoaded: false,
    sending: false,
    customPromptOverride: null, // Override system prompt dari admin
    modelOverride: null, // Override model dari admin
  };

  // Default endpoint Mistral
  const DEFAULT_ENDPOINT = 'https://api.mistral.ai/v1/chat';
  const DEFAULT_MODEL = 'mistral-tiny'; // Model default Mistral
  ...
  async function initChatData(force = false) {
    ...
    try {
      ...
      const [keySnap, endpointSnap, cmsSnap, evSnap, promptSnap, modelSnap] = await Promise.all([
        db.ref('settings/mistralKey').once('value'), // Ganti dari geminiKey ke mistralKey
        db.ref('settings/mistralEndpoint').once('value'),
        db.ref('cms/landing').once('value'),
        db.ref('events').once('value'),
        db.ref('settings/chatbotSystemPrompt').once('value'),
        db.ref('settings/chatbotModel').once('value'),
      ]);

      STATE.mistralKey = keySnap.val() || null;
...
- API key Mistral, Firebase config, atau konfigurasi teknis apapun
...
- Jika tidak yakin dengan jawaban, sarankan user untuk menghubungi admin Ryusei Agency & Event Manajemen. langsung`;
  ...
  async function sendChat() {
    ...
    STATE.history.push({ role: 'user', content: enrichedText });
    ...
    try {
      // Tentukan endpoint dan model
      const endpoint = STATE.endpoint || DEFAULT_ENDPOINT;
      const model = STATE.modelOverride || DEFAULT_MODEL;

      const res = await fetch(endpoint, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${STATE.mistralKey}`,
        },
        body: JSON.stringify({
          model: model,
      ...
      if (e.message && e.message.includes('quota')) {
        appendMsg('bot', 'Kuota API Mistral sedang habis. Silakan coba beberapa saat lagi. 🙏');
      } else if (e.message && e.message.includes('network')) {
        appendMsg('bot', 'Tidak ada koneksi internet. Periksa koneksimu dan coba lagi. 🌐');
      } else if (e.message && e.message.includes('model_not_found')) {
        appendMsg('bot', 'Model Mistral tidak tersedia. Silakan hubungi admin. 🙏');
      } else {
  ...
  window.RyuseiChat = {
    ...
    initChatData,
    buildSystemPrompt,
    getState: () => ({ ...STATE, mistralKey: STATE.mistralKey ? '***hidden***' : null }),
  };
